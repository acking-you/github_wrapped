...
#
# Usage:
#   YEAR=2025 ./scripts/collect_raw.sh
#   YEAR=2025 GH_USER=octocat ./scripts/collect_raw.sh
#
# Optional:
#   OUT_BASE=data/github-wrapped-2025   # default: data/github-wrapped-$YEAR
...
#
# NOTE: Use GH_USER instead of USER to avoid clobbering the system environment variable.
...
echo "Collecting raw data: user=${GH_USER} year=${YEAR}" >&2

# Snapshot query files for audit/replay.
...
gh api graphql \
  -f query="$(cat "${QUERY_DIR}/contributions.graphql")" \
  -f user="${GH_USER}" \
  -f from="${FROM}" \
  -f to="${TO}" \
  > "${RAW_DIR}/contributions.json"
...
if [[ "${PREV_YEARS}" != "0" ]]; then
  for ((i=1; i<=PREV_YEARS; i++)); do
    ...
    gh api graphql \
      ...
      > "${RAW_DIR}/contributions_${y}.json"
  done
fi

# 3) Starred repos (GraphQL, paginated pages)
gh api graphql --paginate --slurp \
  -f query="$(cat "${QUERY_DIR}/starred_repos.graphql")" \
  -f user="${GH_USER}" \
  > "${RAW_DIR}/starred_repos_pages.json"

# 4) Merged PRs in the year (GraphQL Search, paginated pages)
SEARCH_QUERY="author:${GH_USER} is:pr is:merged merged:${YEAR}-01-01..${YEAR}-12-31"

gh api graphql --paginate --slurp \
  -f query="$(cat "${QUERY_DIR}/prs_merged.graphql")" \
  -f queryString="${SEARCH_QUERY}" \
  > "${RAW_DIR}/prs_${YEAR}_pages.json"

# 5) Contributed repos (GraphQL, paginated pages)
gh api graphql --paginate --slurp \
  -f query="$(cat "${QUERY_DIR}/contributed_repos.graphql")" \
  -f user="${GH_USER}" \
  > "${RAW_DIR}/contributed_repos_pages.json"

# 6) User repos (REST, paginate)
gh api "users/${GH_USER}/repos?per_page=100" --paginate > "${RAW_DIR}/user_repos.json"

# 7) Recent events (REST, best-effort only)
FETCH_EVENTS_90D="${FETCH_EVENTS_90D:-1}"
if [[ "${FETCH_EVENTS_90D}" == "1" ]]; then
  if ! gh api "users/${GH_USER}/events?per_page=100" --paginate > "${RAW_DIR}/events_90d.json"; then
